# WASP Software Engineering Course Assignment 2026

<!--
- deadline: 2026-09-01T14:00
- 4–6 A4 pages
- 11pt Times New Roman
-->

## Introduction

<!-- introduce your research area (≤500 words) -->

## Lecture Principles

<!-- two principles -->

### Discussion

## Guest-Lecture Principles

<!-- two principles -->

### Discussion

## Data Scientists, Software Engineers, and AI Engineers

<!--
- read chapters 1&2 of <https://mlip-cmu.github.io/book/>
- how does the book differentiate between data scientists and software engineers?
- do you agree?
- production ML requires system-wide view (not only model-centric)
  - apply this to your research area
- will data scientists and software engineers remain distinct?
  - what about AI engineering?
  - is it software engineering for AI, systematic DS, MLOps, or something else?
  - discuss the role of foundation models, AI coding assistants, RAGs, prompt pipelines, and agentic workflows
-->

## Paper Analysis

<!--
- core ideas, importance to SE
- relation to your research
- application to an example AI project
- adapt these ideas to your research
- 1 A4 page each
-->

### Paper 1

<!-- CAIN conference <https://conf.researchr.org/series/cain> -->

### Paper 2

<!--
one of the following
- LLMs for software engineering
- AI-assisted development
- LLM/RAG/prompt evaluation
- testing of ML/AI systems
- agentic software engineering
- AI system security/safety
- technical debt in AI systems
- software engineering for foundation-model-based systems
-->

## Evidence, Originality, and GenAI-Use

<!--
for each source
- describe type of source
- comment on trustworthiness of sources
- dislose genAI use
-->
